...
```less
✓ Select an authentication method GoogleAuthenticating to Prisma Platform via browser.Visit the following URL in your browser to authenticate:https://console.prisma.io/auth/cli?state=eyJjb6ll...Successfully authenticated as amanyoyoyo@gmail.com.Let's set up your Prisma Postgres database!✓ Select your region: ap-southeast-1 - Asia Pacific (Singapore)✓ Enter a project name: My Prisma Project✓ Success! Your Prisma Postgres database is ready ✅We found an existing schema.prisma file in your current project directory.--- Database URL ---Connect Prisma ORM to your Prisma Postgres database with this URL:--- Next steps ---Go to https://pris.ly/ppg-init for detailed instructions.1. Install the Postgres adapternpm install @prisma/adapter-pg...and add it to your Prisma Client instance:import { PrismaClient } from "./generated/prisma/client";import { PrismaPg } from "@prisma/adapter-pg";const connectionString = `${process.env.DATABASE_URL}`;const adapter = new PrismaPg({ connectionString });const prisma = new PrismaClient({ adapter });2. Apply migrationsRun the following command to create and apply a migration:npx prisma migrate dev3. Manage your dataView and edit your data locally by running this command:npx prisma studio...or online in Console:https://console.prisma.io/cmhyn0uwl0q6903foel16ff31/cmhyn143t074tyLfoezs684ag/cmhyn143t074uylfon8hfre5z/studio4. Send queries from your appIf you already have an existing app with Prisma ORM, you can now run it and it will send queries against your newly created Prisma Postgres instance.5. Learn moreFor more info, visit the Prisma Postgres docs: https://pris.ly/ppg-docs
```
...
```bash
datasource db {  provider = "postgresql"}generator client {  provider        = "prisma-client"  previewFeatures = ["metrics"]}
```
...
```bash
datasource db {  provider = "postgresql"}generator client {  provider        = "prisma-client"  previewFeatures = ["views", "metrics"]}
```
...
```perl
// This is your Prisma schema file,// learn more about it in the docs: https://pris.ly/d/prisma-schemagenerator client {  provider = "prisma-client"  output   = "../generated/prisma"}datasource db {  provider = "postgresql"}
```
...
```ruby
# Environment variables declared in this file are automatically made available to Prisma.# See the documentation for more detail: https://pris.ly/d/prisma-schema#using-environment-variables# Prisma supports the native connection string format for PostgreSQL, MySQL, SQLite, SQL Server, MongoDB and CockroachDB.# See the documentation for all the connection string options: https://pris.ly/d/connection-stringsDATABASE_URL="postgresql://user:password@localhost:5432/mydb"
```
...
```graphql
node_modules# Keep environment variables out of version control.env/generated/prisma
```
...
```perl
// This is your Prisma schema file,// learn more about it in the docs: https://pris.ly/d/prisma-schemadatasource db {  provider = "mysql"}generator client {  provider = "prisma-client"  output   = "../generated/prisma"}
```
...
```javascript
✔ Generated Prisma Client to ./node_modules/.prisma/client in 61msYou can now start using Prisma Client in your code:import { PrismaClient } from '../prisma/generated/client'// or const { PrismaClient } = require('@prisma/client')const prisma = new PrismaClient()Explore the full API: https://pris.ly/d/client
```
...
The `prisma-client` generator creates a customized client for working with your database in a custom output directory specified by the `output` field - you can [customize the output folder](https://www.prisma.io/docs/orm/prisma-client/setup-and-configuration/generating-prisma-client#using-a-custom-output-path).
...
## `dev`[](https://www.prisma.io/docs/orm/reference/prisma-cli-reference#dev "Direct link to dev")
...
```csharp
$ npx prisma devFetching latest updates for this subcommand...✔  Great Success! 😉👍   Your  prisma dev  server default is ready and listening on ports 63567-63569.╭──────────────────────────────╮│[q]uit  [h]ttp url  [t]cp urls│╰──────────────────────────────╯
```

### `dev stop`[](https://www.prisma.io/docs/orm/reference/prisma-cli-reference#dev-stop "Direct link to dev-stop")
...
### `dev rm`[](https://www.prisma.io/docs/orm/reference/prisma-cli-reference#dev-rm "Direct link to dev-rm")
...
```graphql
Introspecting based on datasource defined in schema.prisma …✔ Introspected 2 models and wrote them into schema.prisma in 38msRun prisma generate to generate Prisma Client.
```
...
```graphql
Introspecting based on datasource defined in alternative/schema.prisma …✔ Introspected 2 models and wrote them into alternative/schema.prisma in 60msRun prisma generate to generate Prisma Client.
```
...
```kotlin
generator client {  provider = "prisma-client"  output   = "./generated"}datasource db {  provider = "sqlite"  url      = "file:./hello-prisma.db"}model User {  email   String    @unique  name    String?  user_id Int       @id @default(autoincrement())  post    Post[]  profile Profile[]}model Post {  content   String?  post_id   Int     @id @default(autoincrement())  title     String  author    User?   @relation(fields: [author_id], references: [user_id])  author_id Int?}model Profile {  bio        String?  profile_id Int     @id @default(autoincrement())  user       User    @relation(fields: [user_id], references: [user_id])  user_id    Int     @unique}
```
...
#### AI Safety guardrails for `prisma migrate reset`[](https://www.prisma.io/docs/orm/reference/prisma-cli-reference#ai-safety-guardrails-for-prisma-migrate-reset "Direct link to ai-safety-guardrails-for-prisma-migrate-reset")
...
### `mcp`[](https://www.prisma.io/docs/orm/reference/prisma-cli-reference#mcp "Direct link to mcp")

Starts the [Prisma MCP server](https://www.prisma.io/docs/postgres/integrations/mcp-server).
...
For detailed database support information, including SQLite requirements, see [Databases supported by Prisma Studio](https://www.prisma.io/docs/orm/tools/prisma-studio#databases-supported-by-prisma-studio).
...
#### Arguments[](https://www.prisma.io/docs/orm/reference/prisma-cli-reference#arguments-13 "Direct link to Arguments")
...
#### Examples[](https://www.prisma.io/docs/orm/reference/prisma-cli-reference#examples-16 "Direct link to Examples")
...
#### Start Studio with a custom Prisma config file[](https://www.prisma.io/docs/orm/reference/prisma-cli-reference#start-studio-with-a-custom-prisma-config-file "Direct link to Start Studio with a custom Prisma config file")

```bash
prisma studio --config=./prisma.config.ts
```

#### Start Studio with a direct database connection string[](https://www.prisma.io/docs/orm/reference/prisma-cli-reference#start-studio-with-a-direct-database-connection-string "Direct link to Start Studio with a direct database connection string")
...
## `npx create-db`[](https://www.prisma.io/docs/orm/reference/prisma-cli-reference#npx-create-db "Direct link to npx-create-db")
...
For full usage details, options (such as `--region` and `--interactive`), and examples, see the [documentation](https://www.prisma.io/docs/postgres/introduction/npx-create-db).