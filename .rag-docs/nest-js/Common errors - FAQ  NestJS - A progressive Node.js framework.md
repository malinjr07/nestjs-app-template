...
#### "Cannot resolve dependency" error[#](https://docs.nestjs.com/faq/common-errors#cannot-resolve-dependency-error)

> **Hint** Check out the [NestJS Devtools](https://docs.nestjs.com/devtools/overview#investigating-the-cannot-resolve-dependency-error) which can help you resolve the "Cannot resolve dependency" error effortlessly.
...
The most common culprit of the error, is not having the `<provider>` in the module's `providers` array. Please make sure that the provider is indeed in the `providers` array and following [standard NestJS provider practices](https://docs.nestjs.com/fundamentals/custom-providers#di-fundamentals).
...
1.  you're importing the class reference or use a custom token with `@Inject()` decorator. Read the [custom providers page](https://docs.nestjs.com/fundamentals/custom-providers), and
...
#### "Circular dependency" error[#](https://docs.nestjs.com/faq/common-errors#circular-dependency-error)
...
#### Debugging dependency errors[#](https://docs.nestjs.com/faq/common-errors#debugging-dependency-errors)
...
#### "File change detected" loops endlessly[#](https://docs.nestjs.com/faq/common-errors#file-change-detected-loops-endlessly)
...
This tells TypeScript to use the polling method for checking for file changes instead of file system events (the new default method), which can cause issues on some machines. You can read more about the `"watchFile"` option in [TypeScript documentation](https://www.typescriptlang.org/tsconfig#watch-watchDirectory).